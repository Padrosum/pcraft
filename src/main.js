...
function initWorld(seed, edits, time) {
...
function startGame(name) {
  ...
  net = connect(roomId, {
    ...
    onState: (data) => initWorld(data.seed, data.edits, data.time),
  ...
  if (!isJoiner) {
    ...
    if (save) initWorld(save.seed, save.edits, save.time);
    else initWorld((Math.random() * 0xffffffff) | 0, [], 0.08);
  } else {
